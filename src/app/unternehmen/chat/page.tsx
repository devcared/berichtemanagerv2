...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/company/chat')
      ...
      .then(json => { setMessages(json.messages ?? []); setLoading(false) })
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`chat:${profile.companyId}`)
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'chat_messages', filter: `company_id=eq.${profile.companyId}` },
        async (payload) => {
          const raw = payload.new as { id: string; company_id: string; sender_id: string; content: string; created_at: string }
          // Fetch sender name
          ...
          <div key={group.date}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0 8px' }}>
              ...
              <span style={{ fontSize: '0.6875rem', color: 'hsl(var(--muted-foreground))', fontWeight: 500 }}>
                {format(new Date(group.date), 'EEEE, d. MMMM', { locale: de })}
              </span>
              <div style={{ flex: 1, height: 1, background: 'hsl(var(--border))' }} />
            </div>
